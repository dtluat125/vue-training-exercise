...
const rangePriceOptions = computed(() => {
  ...
  for (i = 0; i < max; i += 100) {
    options.push({
      ...
      label: `$${numberWithCommasFormat2(i)} - $${numberWithCommasFormat2(
        i + 100
      )}`,
    });
  }
  ...
  options.push({
    ...
    label: `$${numberWithCommasFormat2(i)} And Above`,
  });
...
const filteredTags = computed(() => {
  ...
  if (rangePrice.value.min || rangePrice.value.max)
    tags.push({
      type: "price",
      value: rangePrice,
      label: `$${numberWithCommasFormat2(rangePrice.value.min)} - ${
        rangePrice.value.max
          ? `$${numberWithCommasFormat2(rangePrice.value.max)}`
          : "above"
      }`,
    });
  return tags;
